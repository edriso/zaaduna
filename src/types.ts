...
export interface MessageSchedule extends BaseSchedule {
  ...
  /**
   * Optional day-alternating card image. Variant 1 on even civil days, variant
   * 2 on odd ones (see content/cards.ts → cardVariantFor), so all three azkar
   * share the day's variant and consecutive days alternate. It is sent as a
   * SILENT separate photo just before the text — Telegram's 1024-char photo
   * caption can't hold the full azkar — and replaced on the next fire like the
   * text (tracked under a `${name}::card` state key, so the text ring buffer is
   * untouched). A send failure is logged and non-fatal (the text still posts).
   * Paths resolve from the project root (cwd). See scheduler.ts#sendCard.
   */
